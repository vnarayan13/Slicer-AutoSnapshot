...
def getDataFiles(selectedDir,SubFolders=False,Mask=False,selImage=False,selLabel='label'):
    ...
    for root, dirs, filenames in os.walk(selectedDir):
        [dataFilePaths.append(str(os.path.join(root,str(file)))) for file in fnmatch.filter(filenames, str(Mask))]#files if file.endswith(Mask.strip("*"))]
...
def getFileList(selectedDir,SubFolders=False,Mask='*'):
    ...
    if SubFolders:    
        ...
        files = []
        for root, dirs, filenames in os.walk(selectedDir):
            [files.append(str(os.path.join(root,str(file)))) for file in fnmatch.filter(filenames, str(Mask))]
    else:
        # Get all files in current dir
        files = selDir.files(Mask)
        #allFiles = [f for f in os.listdir(selectedDir) if os.path.isfile(f)]
        #[files.append(fi) for fi in in fnmatch.filter(allfiles, str(Mask))]
    return files
